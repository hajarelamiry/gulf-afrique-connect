...
const services = [
  { icon: GraduationCap, title: "Talents Certifiés", desc: "Un réservoir de PhD, chercheurs et ingénieurs seniors (Top 5-10%) rigoureusement sélectionnés via un processus en cinq étapes." },
  { icon: BrainCircuit, title: "Conseil Stratégique Élite", desc: "Accès rapide à des connaissances stratégiques mondiales et des décideurs pour la due diligence technologique." },
  { icon: FlaskConical, title: "Exécution R&D Flexible", desc: "Main-d'œuvre scientifique pour l'analyse de données, la modélisation et l'ingénierie à des coûts compétitifs." },
  { icon: FileCheck, title: "Conformité & PI", desc: "Cadre contractuel sécurisé garantissant la cession de la propriété intellectuelle et le respect des standards de qualité (SLA)." },
];
        ...
        {services.map((s, i) => (
          <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i + 3} variants={fadeUp}
            ...
            <div className="w-12 h-12 rounded-xl bg-secondary flex items-center justify-center mb-5 group-hover:bg-primary transition-colors duration-300">
              <s.icon size={22} className="text-white transition-colors duration-300" />
            </div>
